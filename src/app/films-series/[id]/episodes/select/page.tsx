...
export default function SelectEpisodePage() {
  ...
  function handleSelectEpisode(epId: string, epNumber: number) {
    const hasPreviousUnwatched = episodes
      .filter((e) => e.episodeNumber < epNumber)
  ...
  return (
    <div className="relative flex min-h-dvh w-full flex-col bg-white">
      ...
      >
        {/* Subtitel */}
        <p className="text-base font-medium leading-6 text-[var(--text-primary)]">
          Selecteer de aflevering die je wil markeren als bekeken
        </p>
        ...
        <div className="flex flex-col gap-3">
          {loading
            ...
            : episodes.map((ep) => {
                ...
                const hasEyeIcon = ep.episodeNumber > 1;
                return (
                  <button
                    key={ep.episodeNumber}
                    type="button"
                    onClick={() => handleSelectEpisode(epId, ep.episodeNumber)}
                    className={cn(
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          {hasEyeIcon && (
                            <MaskIcon
                              ...
                              className={cn("size-6 shrink-0", epWatched ? "bg-[#34C759]" : "bg-[#4f55f1]")}
                            />
                          )}
                        </div>
                        {meta && (
      ...
      {/* Slide-in: markeer deze of alle voorgaande afleveringen */}
      <SlideInModal
        open={pendingEp !== null}
