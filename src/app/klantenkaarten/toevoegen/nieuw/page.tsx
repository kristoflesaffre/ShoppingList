...
function KlantenkaartNieuwScanInner() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const cardName = React.useMemo(() => {
    const raw = searchParams.get("naam") ?? "";
    return raw.trim().slice(0, MAX_NAAM_LEN);
  }, [searchParams]);
  ...
  React.useEffect(() => {
    ...
    if (!cardName) router.replace("/klantenkaarten/toevoegen");
  }, [authLoading, user, cardName, router]);
...
export default function KlantenkaartNieuwPage() {
  return (
    <Suspense fallback={<PageSpinner surface="transparent" />}>
      <KlantenkaartNieuwScanInner />
    </Suspense>
  );
}
